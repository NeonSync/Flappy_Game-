...
# installation fro random pipe appearence 
def spawn_pipe():
    gap_y = random.randint(100, HEIGHT - 100)
    top_pipe = pygame.Rect(WIDTH, 0, pipe_width, gap_y - pipe_gap // 2)
    bottom_pipe = pygame.Rect(WIDTH, gap_y + pipe_gap // 2, pipe_width, HEIGHT - (gap_y + pipe_gap // 2))
    return top_pipe, bottom_pipe

# Checking for coliision of bird 
def check_collision():
...
def update_loop():
    ...
    if countdown is not None:
        if countdown > 0:
            countdown -= 1 / FPS  # Decrease by 1/FPS seconds per frame
    ...
    if countdown is not None:
        countdown_text = font.render(f"{int(countdown) if countdown > 0 else 'Go!'}", True, TEXT_COLOR)
        screen.blit(countdown_text, (WIDTH // 2 - countdown_text.get_width() // 2, HEIGHT // 2))
    elif game_over:
        if score >= win_score:
            end_text = font.render(f"You Win! Score: {score} Press Space", True, TEXT_COLOR)
        else:
            end_text = font.render(f"Game Over! Score: {score} Press Space", True, TEXT_COLOR)
